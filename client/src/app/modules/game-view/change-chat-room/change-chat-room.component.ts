...
export class ChangeChatRoomComponent implements OnInit {
  ...
  constructor(private chatRoomIndexService : ChatRoomIndexService, public changeChatRoomDialogRef: MatDialogRef<ChangeChatRoomComponent>, public chatRoomService : ChatRoomService, private authService : AuthService, private clientSocketService : ClientSocketService) { 
    ...
    if (this.chatRoomIndexService.firstSelection == false && this.userChatsRooms.length > 0) {
      this.chatRoomIndexService.chatRoomIndex = this.chatRoomService.chatRooms.findIndex((chatRoom) => chatRoom.chatRoomName === this.userChatsRooms[0].chatRoomName);
    }
  ...
  changeChatRoom() {
    ...
    if(this.selectedChatRoom) {
        this.selectedChatRoomIndex = this.chatRoomService.chatRooms.findIndex((chatRoom) => chatRoom.chatRoomId === this.selectedChatRoom?.chatRoomId); 
  ...
  disableJoinCurrentRoom(chatRoom : ChatRoom) {
    ...
    return this.chatRoomService.chatRooms.findIndex((chatRoomInService) => chatRoomInService.chatRoomName === chatRoom.chatRoomName) === this.chatRoomIndexService.chatRoomIndex;
  }

  leaveChatRoom(chatRoom : ChatRoom) {
    const chatRoomIndexInService = this.chatRoomService.chatRooms.findIndex((chatRoomInService) => chatRoomInService.chatRoomName === chatRoom.chatRoomName);
